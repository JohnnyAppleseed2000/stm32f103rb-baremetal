...
typedef struct
{
	...
	uint8_t 		*pTxBuffer; // Tx Buffer 주소
	uint8_t 		*pRxBuffer;	// Rx Buffer 주소
	uint32_t 		TxLen;		// Tx 길이
	uint32_t 		RxLen;		// Rx 길이
	uint8_t 		TxRxState;	// Tx Rx 상태
	uint8_t 		DevAddr;	// 기기 주소
    uint32_t        RxSize;		
    uint8_t         Sr;			
}I2C_Handle_t;
